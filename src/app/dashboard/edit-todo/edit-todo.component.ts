...
//initialize the Todo data
  ...
  getToDo = () => {
    if(this.TodoId==null){
      ...
      this.AppService.getToDo(this.TodoId)
      .subscribe((apiResponse) => {
        if (apiResponse.status == 200) {
          if(Cookie.get('receiverId') != apiResponse.data.userId){
  ...
  editToDo = () => {
    if (this.todolist.length == 0 || this.todolist == null || this.todolist == undefined) {
      ...
      this.AppService.editToDo(data)
        .subscribe((apiResponse) => {
          if (apiResponse.status === 200) {
            this.Toastr.successToastr('Edited Sucessfully')
            this.sendGroupNotification(this.TodoId,`${this.userName} has edited the ToDo ${apiResponse.data.title} `)
            this.sendChanges()
    ...
    if (!this.todo.historyToDoId) {
      ...
      this.AppService.undoAction(data)
        .subscribe((apiResponse) => {
          ...
          this.sendGroupNotification(this.TodoId,`${this.userName} has undo action of a ToDo ${apiResponse.data.title} `)
